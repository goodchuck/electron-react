...
import { Viewer } from "../JS/object.js";
...
class TransformationTool {
  ...
  initializeFocusHandles() {
    //let sgBox = new THREE.THREE.BoxGeometry(1, 1, 1);
    let sgPlane = new THREE.PlaneGeometry(0.5, 0.5, 1, 1);
    let sgLowPolySphere = new THREE.SphereGeometry(1, 16, 16);

    let texture = new THREE.TextureLoader().load(`/img/eye_2.png`);
    ...
    for (let handleName of Object.keys(this.focusHandles)) {
      ...
      //node.lookAt(new THREE.THREE.Vector3().addVectors(node.position, new THREE.THREE.Vector3(...align)));
      node.lookAt(new THREE.Vector3(...align));

      let off = 0.9;
      let off_t = this.viewer.cube.scale.x / 2;
      if (align[0] === 1) {
        // node.position.set(1, off, -off).multiplyScalar(0.5);
        node.position.set(off_t, off, -off);
        node.rotation.z = Math.PI / 2;
      } else if (align[0] === -1) {
        node.position.set(-off_t, -off, -off);
        node.rotation.z = Math.PI / 2;
      } else if (align[1] === 1) {
        node.position.set(-off, off_t, -off);
        node.rotation.set(Math.PI / 2, Math.PI, 0.0);
      } else if (align[1] === -1) {
        node.position.set(off, -off_t, -off);
        node.rotation.set(Math.PI / 2, 0.0, 0.0);
      } else if (align[2] === 1) {
        node.position.set(off, off, off_t);
      } else if (align[2] === -1) {
        node.position.set(-off, off, -off_t);
      }
      ...
      //let outlineMaterial = new THREE.THREE.MeshBasicMaterial({
      //	color: 0x000000,
      //	side: THREE.BackSide,
      //	opacity: 0,
      //	transparent: true});
      ...
      //let outline = new THREE.THREE.Mesh(sgPlane, outlineMaterial);
      //outline.scale.set(1.4, 1.4, 1.4);
      //outline.name = `${handleName}.outline`;
      //box.add(outline);
      ...
      pickSphere.addEventListener("click", (e) => {
        ...
        e.target.parent.material.opacity = 1;
        let selected = this.viewer.cube;
        console.log("click!", pickSphere, e);
        ...
        moveTo(this.viewer, newCamPos, newCamTarget);
      });
      ...
      pickSphere.addEventListener("mouseleave", (e) => {
        e.target.parent.material.opacity = 0.7;
        // box.setOpacity(0.4);
      });
    }
  }
  ...
  //         //	//let a = this.viewer.scene.getActiveCamera().getWorldDirection(new THREE.THREE.Vector3()).dot(pickVolume.getWorldDirection(new THREE.THREE.Vector3()));
  //         //	console.log(pickVolume.getWorldDirection(new THREE.THREE.Vector3()));
  //         //});
  ...
  //         //this.debug = new THREE.THREE.Object3D();
  ...
  //         handle = drag.handle;
  //     }
  //     console.log("e drag scale 2",e,this.scene, camera,handle);
  //     this.dragging = true;
  ...
  //         if (I) {
  //             let iOnLine = drag.line.closestPointToPoint(I, false, new THREE.Vector3());
  //             console.log("iOnLine",iOnLine);
  //             let direction = handle.alignment.reduce( (a, v) => a + v, 0); // 방향 1 or -1
  //             let toObjectSpace = this.selection[0].matrixWorld.clone().invert(); // 역행렬?
  ...
  //                 dragDirectionOS.set(0, 0, 0);
  //             }
  //             let dragDirection = dragDirectionOS.dot(new THREE.Vector3(...handle.alignment)); //dot은 내적
  ...
  update() {
    ...
    for (let handleName of Object.keys(this.handles)) {
      ...
      let scale = new THREE.Vector3(
        this.viewer.cube.scale.x,
        this.viewer.cube.scale.y,
        this.viewer.cube.scale.z
      );
      ...
      // console.log("node child",node.children);
      // if(handleName === "focus.x+"){
      //     node.position.set(cube.position.x + 2,0,0);

      // }
      // else if(handleName === "focus.x-"){
      //     node.position.set(cube.position.x - 2,0,0);
      // }
      // else if(handleName === "focus.y+"){
      //     node.position.set(0,cube.position.y + 2,0);
      // }
      // else if(handleName === "focus.y-"){
      //     node.position.set(0,cube.position.y - 2,0);
      // }
      // else if(handleName === "focus.z+"){
      //     node.position.set(0,0,cube.position.z + 2);
      // }
      // else if(handleName === "focus.z+"){
      //     node.position.set(0,0,cube.position.z - 2);
      // }
      ...
      // console.log("node, cube",handleName,cube);
    }
  }
}

function moveTo(viewer, endPosition, endTarget) {
  let camera = viewer.camera;
  console.log("실행", TWEEN);
  // let view = scene.view;
...
// document.getElementById('testCanvas').addEventListener('dblclick', )
...
//     const intersects = raycaster.intersectObject(scene.child)
...
// function init() {
//     TfT = new TransformationTool();
//     console.log("init transformation")
//     // document.getElementById("testbutton").addEventListener("click", function() {
//     //     console.log("this",TfT);
//     //     TfT.pickVolumes[0].dispatchEvent({type: 'click',test:'tt'});
//     // })

//     // document.getElementById("testbutton_y").addEventListener("click", function() {
//     //     console.log("this",TfT);
//     //     TfT.pickVolumes[2].dispatchEvent({type: 'click',test:'tt'});
//     // })

//     // document.getElementById("testbutton_z").addEventListener("click", function() {
//     //     console.log("this",TfT);
//     //     TfT.pickVolumes[4].dispatchEvent({type: 'click',test:'tt'});
//     // })

// }

var test;
export { test, TransformationTool };
